...
class Tree(object):
    def __init__(self, start, goal, obstacles, xmin, ymin, xmax, ymax, maxNumNodes=1000, res=0.0001, eta=1., gamma=20.,
                 epsilon=0.5):
        ...
        for i in range(neighbour_indices.shape[0]):
            ...
            if new_costs[i] < self.nodes[neighbour_indices[i], 2]:
                ...
                while len(self.update_q) != 0:
                    ...
                    self.nodes[child_index, 2] = self.nodes[parent_index, 2] + np.linalg.norm(
                        dist)  # update child's cost
    ...
    def selectBranch(self, solnpathIDs):
        ...
        self.nodes, subpathIDs, self.goalIDs = self.rerootAtID(self.pcurID, tree=self.nodes, pathIDs=solnpathIDs,
    ...
    def validPath(self, solPathID):
        ...
        if not (np.any(mask)):  # assert that solpath is in collision
            ...
            mask2 = [not self.isValidBranch(self.nodes[solPathID[i], 0:2], self.nodes[solPathID[i + 1], 0:2],
                                            np.linalg.norm(
                                                self.nodes[solPathID[i], 0:2] - self.nodes[solPathID[i + 1], 0:2])) for
                     i in range(solPathID[:-1].shape[0])]
        ...
        allDeadNodesID = \
        np.argwhere([not self.collisionFree(self.nodes[i, 0:2]) for i in range(np.shape(self.nodes)[0])]).reshape(1,
                                                                                                                  -1)[0]
        allDeadNodesID = np.delete(allDeadNodesID, np.argwhere(allDeadNodesID == self.pcurID))
        deadNodesID = list(set(deadNodesID) | set(
            allDeadNodesID))  # union the 2 sets in case nodes inbetween in-collisions have to be removed as well
        # 3. Extract orphan subtree and separate_path to goal
        print("EXTRACTING SUBTREE >>>>")
        self.orphanedTree, self.separatePathID, orphanGoalIDs = self.rerootAtID(p_separateID, self.nodes, solPathID,
                                                                                self.goalIDs)
    ...
    def adoptTree(self, parentNodeID, orphanedTree):
        ...
        orphanedTree[np.where(orphanedTree[:, 3] != -1), 3] = orphanedTree[np.where(orphanedTree[:, 3] != -1), 3] + \
                                                              np.shape(self.nodes)[0]
        ...
        fullTree[orphanRootNewID, 2] = fullTree[parentNodeID, 2] + np.linalg.norm(
            fullTree[parentNodeID, 0:2] - fullTree[orphanRootNewID, 0:2])
