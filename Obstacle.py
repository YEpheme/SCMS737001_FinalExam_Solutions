...
class Obstacle(object):
    ...
    def __init__(self, kind, parameters, velMean, velCovar=np.eye(2) * 0.02, speed=0.5, \
                 borders=[-15, -15, 15, 15], goalLoc=[-14., 13., 0.5]):
    ...
    def rectCollisionFree(self, x):
        ...
        x_check = np.logical_and(x[:, 0] >= self.position[0] - self.robot_r,
                                 x[:, 0] <= self.position[0] + self.width + self.robot_r)
        y_check = np.logical_and(x[:, 1] >= self.position[1] - self.robot_r,
                                 x[:, 1] <= self.position[1] + self.height + self.robot_r)
